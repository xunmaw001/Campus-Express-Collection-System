            ...
            <div class="row breadcrumb-bar">
                ...
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">快递表列表</li>
                    </ol>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                ...
                <div class="col-md-12">
                    <div class="widget-area-2 lochana-box-shadow">
                        <h3 class="widget-title">快递表列表</h3>
                        <div class="table-responsive mb-3">
                            ...
                            <table id="tableId" class="table table-bordered table-striped">
                                ...
                                <tr>
                                    ...
                                    <th onclick="sort('name')">快递名称</th>
                                    <th onclick="sort('consignee')">收件人</th>
                                    <th onclick="sort('consignee')">收件地址</th>
                                    <th onclick="sort('consignee')">收件人手机号</th>
                                    <th onclick="sort('addresser')">发件人</th>
                                    <th onclick="sort('addresser')">发件人手机号</th>
                                    <th onclick="sort('kdztTypes')">快递状态</th>
                                    <th onclick="sort('kdztTypes')">取件码</th>
                                    <th onclick="sort('createTime')">快递发布时间</th>
                                    <th>操作</th>
                                </tr>
                                </thead>
                                <tbody>
...
<script>
    ...
    function setDataRow(item, number) {
        ...
        var addresserCell = document.createElement('td');
        addresserCell.innerHTML = item.addresser;
        row.appendChild(addresserCell);
        var addresserCell = document.createElement('td');
        addresserCell.innerHTML = item.addresserNumber;
        row.appendChild(addresserCell);
        ...
        if(item.kdztTypes == 1){
            ...
            var detailAttr = "Issuepickuporder(" + item.id + ')';
            detailBtn.setAttribute("type", "button");
            detailBtn.setAttribute("class", "btn btn-info btn-sm 发布代取订单");
    ...
    // 删除
    function Issuepickuporder(id) {
        var mymessage = confirm("真的要发布代取订单吗？");
        if (mymessage == true) {
                paramArray = id;
            httpJson("daiqundingdan/Issuepickuporder", "POST", paramArray, (res) => {
                if(res.code == 0)
            {
                getDataList();
                alert('发布成功');
    ...
    function crossTable(id, crossTableName) {
        ...
        window.sessionStorage.setItem('crossTableName', "kuaidi");
