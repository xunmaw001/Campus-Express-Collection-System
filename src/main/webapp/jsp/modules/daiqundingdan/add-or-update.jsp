    ...
    <div id="content">
        ...
        <div class="container mt-0">
            <div class="row breadcrumb-bar">
                <div class="col-md-6">
                    <h3 class="block-title">编辑代取订单表</h3>
                </div>
                <div class="col-md-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">编辑代取订单表</li>
                    </ol>
                </div>
            </div>
        </div>
            ...
            <div class="row">
                ...
                <div class="col-md-12">
                    <div class="widget-area-2 lochana-box-shadow">
                        <h3 class="widget-title">代取订单表信息</h3>
                        <form id="addOrUpdateForm">
                            <div class="form-row">
                                   ...
                                   <div class="form-group col-md-6">
                                    ...
                                    <div class="form-group col-md-6">
                                        <label>送件地址</label>
                                        <input id="consigneeSite" name="consigneeSite" class="form-control"
                                               placeholder="送件地址">
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label>收件人</label>
                                        <input id="consignee" name="consignee" class="form-control"
                                               placeholder="收件人">
                                    </div>
                                    <div class="form-group col-md-6">
                                        <label>收件人手机号</label>
                                        <input id="consigneeNumber" name="consigneeNumber" class="form-control"
                                               placeholder="收件人手机号">
                                    </div>
                                   <div class="form-group col-md-6">
                                       <label>快递状态</label>
                                       <select id="kdztTypesSelect" name="kdztTypes" class="form-control">
                                       </select>
                                   </div>
                                   <div class="form-group col-md-6">
                                       <label>代取人</label>
                                       <select id="dqrTypesSelect" name="dqrTypes" class="form-control">
                                       </select>
                                   </div>
...
<script>
    ...
    var vm = new Vue({
        ...
        beforeCreate: function () {
            ...
            if (id != null && id != "" && id != "null") {
                $.ajax({
                    type: "GET",
                    url: baseUrl + "daiqundingdan/info/" + id,
    ...
    function submit() {
        if (validform() == true && compare() == true) {
            ...
            httpJson("daiqundingdan/" + urlParam, "POST", data, (res) => {
                if(res.code == 0
        )
            {
    ...
    //查询当前页面下所有列表
        ...
        function dqrTypesSelect() {
            //填充下拉框选项
            http("renyuan/page?page=1&limit=100&role=代取员&order=&dicCode=dqr_types", "GET", {}, (res) => {
                if(res.code == 0){
                    dqrTypesOptions = res.data.list;
            }
        });
        }
    ...
    function initializationDqrtypesSelect(){
        var dqrTypesSelect = document.getElementById('dqrTypesSelect');
            for (var i = 0; i < dqrTypesOptions.length; i++) {
                dqrTypesSelect.add(new Option(dqrTypesOptions[i].name,dqrTypesOptions[i].id));
            }
    }
    // 下拉框选项回显
    function setSelectOption() {
        ...
        for (var i = 0; i < kdztTypesOptions.length; i++) {
            if(kdztTypesOptions[i].codeIndex == ruleForm.kdztTypes){//下拉框value对比,如果一致就赋值汉字
                document.getElementById("kdztTypesSelect").options[i].selected = true;
            }
        }
        for (var i = 0; i < dqrTypesOptions.length; i++) {
            if(dqrTypesOptions[i].id == ruleForm.dqrTypes){//下拉框value对比,如果一致就赋值汉字
                document.getElementById("dqrTypesSelect").options[i].selected = true;
            }
        }
    }
    ...
    function getDetails() {
        ...
        if (id != null && id != "" && id != "null") {
            updateId = id;
            window.sessionStorage.removeItem('updateId');
            http("daiqundingdan/info/" + id, "GET", {}, (res) => {
                if(res.code == 0
        )
            {
                ruleForm = res.data
    ...
    function dataBind() {
        ...
        $("#consigneeSite").val(ruleForm.consigneeSite);
        $("#consignee").val(ruleForm.consignee);
        $("#consigneeNumber").val(ruleForm.consigneeNumber);
